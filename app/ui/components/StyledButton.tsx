...
export default function StyledButton(props: StyledButtonProps) {
    ...
    return (
        <button
            className={`${styleClasses[props.style || "filled"][props.variant || "primary"]} w-fit shrink-0 rounded px-4 py-2 ${nunito.className} cursor-pointer font-bold transition-colors ${extraClasses}`}
            onClick={onClick}
        >
            {children}
        </button>
