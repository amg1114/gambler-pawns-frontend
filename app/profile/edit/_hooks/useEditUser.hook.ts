...
export function useEditForms() {
   ...
   */
  const sendUserDetailUpdates = async (
    data: Partial<User>,
    session: Session,
   ...
   *
   * @param data - The password form data containing the new password.
   * @param session - The current user session containing the authentication token.
   * @returns A promise that resolves to an object containing the success status and a message.
   *
   * @throws Will throw an error if the server returns a status code of 500.
   */
   ...
   */
  const userDataHasErrors = <T extends z.ZodRawShape>(
